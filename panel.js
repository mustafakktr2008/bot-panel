...
function updateChannel() {
    ...
    if (guilds.val() === "DM") {
        ...
        $("#emoji").html(`${localeFile.text.userId} : (${user.id}) <button class="mini" data-value="<@!${user.id}>" onclick="addText(this.dataset.value)">@</button>`);
...
function sendMessage() {
    ...
    if (toSend.html() === "") {
        tempChange("#gonder", `[${localeFile.errors.emptyMsg}]`, 1000);
    } else {
        ...
        if (guilds.val() === "DM") {
            ...
            client.channels.cache.find((channel) => channel.id === channels.val()).send(formatted).catch(() => {
                tempChange("#gonder", `[${localeFile.errors.missingPermissions}]`, 1000);
            });
        }
        toSend.html("");
    }
...
function selectChannelOnReload(channel) {
    $(`#kanallar option[value="${channel}"]`).prop('selected', true);
    setTimeout(() => {
        refreshChat.click();
    }, 1000);
}
...
$(document).on("change", "#serverlar", () => {
    updateGuild();
});

$(document).on("change", "#kanallar", () => {
    updateChannel();
});
...
send.click(() => {
  console.log("butona tıklandı abeyy");
    sendMessage();
});
